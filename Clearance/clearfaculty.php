...
<?php
$regno=$faculty=$facultycomment="";
$regno_err=$faculty_err=$facultycomment_err="";
...
if (isset($_POST['update'])){
     ...
     if(empty(trim($_POST['faculty']))){
        $faculty_err= "Please enter the status";
    }
    ...
    //validate venue
      if(empty(trim($_POST['facultycomment']))){
        $facultycomment_err= "Please enter the comment";
    }
      
	   else{
            $faculltycomment= trim($_POST['facultycomment']);
                }

                 //validate time
       
// Check input errors before inserting in database// if there are errors, data will not be submitted to the database.
  if(empty($faculty_err) && empty($facultycomment_err) ){
...
//die($sql);
          ?>
<script> alert("Student Cleared Successfully."); window.location='fac.php'</script>
	<?php

	}else{
?>
<script> alert("No field edited!"); window.location='fac.php'</script>
<?php
	}
}
}	
          ...
          <div class="Id">
			...
			<div class="form-group  has-feedback <?php echo (!empty($regno_err)) ? 'has-error' : ''; ?>">
             ...
             <div class="form-group  has-feedback">
                <label>Status:</label>
                <input type="text" name="faculty"class="form-control"  value="<?php echo $value['faculty'];?>"> 
            </div>
          <div class="form-group  has-feedback <?php echo (!empty($facultycomment_err)) ? 'has-error' : ''; ?>">
                <label>Comment:</label>
               <input type="text" name="facultycomment"class="form-control"  value="<?php echo $value['facultycomment'];?>" >
                <span class="help-block"><?php echo $facultycomment_err; ?></span>
            </div> 
