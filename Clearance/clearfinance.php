...
<?php
$regno=$finance=$financecomment="";
$regno_err=$finance_err=$financecomment_err="";

if (isset($_POST['update'])){

     
	  //validate day
     if(empty(trim($_POST['finance']))){
        $finance_err= "Please enter the status";
    }
    ...
    //validate venue
      if(empty(trim($_POST['financecomment']))){
        $financecomment_err= "Please enter the comment";
    }
...
// Check input errors before inserting in database// if there are errors, data will not be submitted to the database.
  if(empty($finance_err) && empty($financecomment_err) ){
      
 // Prepare an update statement
  //$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
 $id=$_POST['id']; 
 
       $sql="UPDATE allsub SET finance ='$finance', financecomment='$financecomment' WHERE id='$id'";
if(mysqli_query($conn,$sql)){
//die($sql);
          ?>
<script> alert("Student Cleared Successfully."); window.location='fin.php'</script>
	<?php

	}else{
?>
<script> alert("No field edited!"); window.location='fin.php'</script>
<?php
	}
}
}	
          ...
          <div class="Id">
			...
			<div class="form-group  has-feedback <?php echo (!empty($regno_err)) ? 'has-error' : ''; ?>">
             ...
             <div class="form-group  has-feedback">
                <label>Status:</label>
                <input type="text" name="finance"class="form-control"  value="<?php echo $value['finance'];?>"> 
            </div>
          <div class="form-group  has-feedback <?php echo (!empty($financecomment_err)) ? 'has-error' : ''; ?>">
                <label>Comment:</label>
               <input type="text" name="financecomment"class="form-control"  value="<?php echo $value['financecomment'];?>" >
                <span class="help-block"><?php echo $financecomment_err; ?></span>
            </div> 
