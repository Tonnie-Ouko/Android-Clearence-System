...
<?php
$regno=$security=$securitycomment="";
$regno_err=$security_err=$securitycomment_err="";
...
if (isset($_POST['update'])){
     ...
     if(empty(trim($_POST['security']))){
        $security_err= "Please enter the status";
    }
    ...
    //validate venue
      if(empty(trim($_POST['securitycomment']))){
        $securitycomment_err= "Please enter the comment";
    }
      
	   else{
            $securitycomment= trim($_POST['securitycomment']);
                }

                 //validate time
       
// Check input errors before inserting in database// if there are errors, data will not be submitted to the database.
  if(empty($security_err) && empty($securitycomment_err) ){
      
 // Prepare an update statement
  //$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
 $id=$_POST['id']; 
 
       $sql="UPDATE allsub SET security ='$security', securitycomment='$securitycomment' WHERE id='$id'";
if(mysqli_query($conn,$sql)){
//die($sql);
          ?>
<script> alert("Student Cleared Successfully."); window.location='sec.php'</script>
	<?php

	}else{
?>
<script> alert("No field edited!"); window.location='sec.php'</script>
<?php
	}
}
}	
          ...
          <div class="Id">
			...
			<div class="form-group  has-feedback <?php echo (!empty($regno_err)) ? 'has-error' : ''; ?>">
             ...
             <div class="form-group  has-feedback">
                <label>Status:</label>
                <input type="text" name="security"class="form-control"  value="<?php echo $value['security'];?>"> 
            </div>
          <div class="form-group  has-feedback <?php echo (!empty($securitycomment_err)) ? 'has-error' : ''; ?>">
                <label>Comment:</label>
               <input type="text" name="securitycomment"class="form-control"  value="<?php echo $value['securitycomment'];?>" >
                <span class="help-block"><?php echo $securitycomment_err; ?></span>
            </div> 
