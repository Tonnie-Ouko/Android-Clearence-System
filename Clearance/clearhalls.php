...
<?php
$regno=$halls=$hallscomment="";
$regno_err=$halls_err=$hallscomment_err="";

if (isset($_POST['update'])){

     
	  //validate day
     if(empty(trim($_POST['halls']))){
        $halls_err= "Please enter the status";
    }
   
     else{
            $halls= trim($_POST['halls']);
                }

    //validate venue
      if(empty(trim($_POST['hallscomment']))){
        $hallscomment_err= "Please enter the comment";
    }
      
	   else{
            $hallscomment= trim($_POST['hallscomment']);
                }

                 //validate time
       
// Check input errors before inserting in database// if there are errors, data will not be submitted to the database.
  if(empty($halls_err) && empty($hallscomment_err) ){
      
 // Prepare an update statement
  //$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
 $id=$_POST['id']; 
 
       $sql="UPDATE allsub SET halls ='$halls', hallscomment='$hallscomment' WHERE id='$id'";
if(mysqli_query($conn,$sql)){
//die($sql);
          ?>
<script> alert("Student Cleared Successfully."); window.location='hal.php'</script>
	<?php

	}else{
?>
<script> alert("No field edited!"); window.location='hal.php'</script>
<?php
	}
}
}	
...
<html>
  <head>
    <meta charset="UTF-8">
    <title>Admin Clearence | Halls</title>
          ...
          <div class="Id">
			...
			<div class="form-group  has-feedback <?php echo (!empty($regno_err)) ? 'has-error' : ''; ?>">
             ...
             <div class="form-group  has-feedback">
                <label>Status:</label>
                <input type="text" name="halls"class="form-control"  value="<?php echo $value['halls'];?>"> 
            </div>
          <div class="form-group  has-feedback <?php echo (!empty($hallscomment_err)) ? 'has-error' : ''; ?>">
                <label>Comment:</label>
               <input type="text" name="hallscomment"class="form-control"  value="<?php echo $value['hallscomment'];?>" >
                <span class="help-block"><?php echo $hallscomment_err; ?></span>
            </div> 
